...
volatile uint32_t TStart = 0x7FFFFFFF;	//Timer起始值
...
{	
	...
	TIMR_Init(TIMR0, TIMR_MODE_TIMER, TStart, 0);
...
void IRQ4_Handler(void)
{
	...
	{
		LWidth = TStart > TIMRG->HALL_A ? TStart - TIMRG->HALL_A : (0x7FFFFFFF + TStart) - TIMRG->HALL_A;
	}
	else											//下降沿
	{
		if(LWidth != 0)								//已测到上升沿
			Period = TStart > TIMRG->HALL_A ? TStart - TIMRG->HALL_A : (0x7FFFFFFF + TStart) - TIMRG->HALL_A;
